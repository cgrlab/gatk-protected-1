 ...
 *
 * @param <S> hidden state type; must implement {@link AlleleMetadataProvider} and {@link CallStringProvider}.
 ...
 */
public class HiddenMarkovModelPostProcessor<D, S extends AlleleMetadataProvider & CallStringProvider & ScalarProvider,
        T extends Target> {
    ...
    public HiddenMarkovModelPostProcessor(
        ...
        if (numSamples == 0) {
            throw new UserException.BadInput("Number of samples must be >= 1");
        }
        if (new HashSet<>(sampleNames).size() < numSamples) {
            throw new UserException.BadInput("Sample names must have unique names");
        }
        if (sampleTargets.size() != numSamples) {
            throw new UserException.BadInput("List of targets per sample must have the same length as the list of" +
                    " sample names");
        }
        if (sampleForwardBackwardResults.size() != numSamples) {
            throw new UserException.BadInput("List of forward-backward result per sample must have the same length" +
                    " as the list of sample names");
        }
        if (sampleBestPaths.size() != numSamples) {
            throw new UserException.BadInput("List of best paths per sample must have the same length as the list" +
                    " of sample names");
        }
        if (IntStream.range(0, numSamples)
                .filter(si -> sampleTargets.get(si).targetCount() != sampleBestPaths.get(si).size()).count() > 0) {
            throw new UserException.BadInput("Some of the provided sample best paths have different lengths than their" +
        ...
        if (IntStream.range(1, numSamples)
                .filter(si -> !new HashSet<>(sampleForwardBackwardResults.get(si).model().hiddenStates()).equals(allStatesInferredSet))
                .count() > 0) {
            throw new IllegalArgumentException("All samples must have the same set of hidden states");
        }
        this.referenceState = referenceState;
        if (!allStatesInferredSet.contains(referenceState)) {
            throw new UserException.BadInput("The provided reference state is not in the HMM hidden states");
        }
        this.alternativeStates = new ArrayList<>(allStatesInferred);
         ...
         */
        this.allStates = new ArrayList<>(allStatesInferred.size());
        ...
        IntStream.range(0, numSamples)
                .forEach(si -> {
                    ...
                    if (!IntStream.range(0, sampleTargetList.size()-1)
                            .allMatch(ti -> IntervalUtils.LEXICOGRAPHICAL_ORDER_COMPARATOR
                                    .compare(sampleTargetList.get(ti + 1), sampleTargetList.get(ti)) > 0)) {
                        throw new UserException.BadInput("The target collection for sample number " + si +
        ...
        allAlleles = allStates.stream()
                .map(AlleleMetadataProvider::toAllele)
                .collect(Collectors.toList());
        alternativeAlleles = alternativeStates.stream()
                .map(AlleleMetadataProvider::toAllele)
                .collect(Collectors.toList());

        /* pool targets from all samples and sort them lexicographically */
        final Set<T> pooledTargetsSet = new LinkedHashSet<>();
        IntStream.range(0, numSamples)
                .forEach(si -> pooledTargetsSet.addAll(sampleTargets.get(si).targets()));
        masterTargetCollection = new HashedListTargetCollection<>(new ArrayList<>(pooledTargetsSet).stream()
    ...
    /**
     * Create the VCF file based on pre-determined segments (though, using the forward-backward result
     * provided to the constructor for calculating various genotyping qualities)
     *
    ...
    private List<GenotypingSegment> composeGenotypingSegments(
        ...
        final List<HiddenStateSegmentRecord<S, T>> records = allSegmentsBySampleName.entrySet()
                .stream()
     ...
     * @param <STATE> type of hidden state; must implement {@link ScalarProvider}
     * @param <TARGET> type of target; must implement {@link Locatable}
     * @throws IllegalStateException if segment length is negative
     * @return segment mean
     */
    public static <STATE extends ScalarProvider, TARGET extends Locatable> double calculateSegmentMean(
            final int firstTargetIndex,
            final int segmentLength,
            final ForwardBackwardAlgorithm.Result<?, TARGET, STATE> fbResult) {
        if (segmentLength < 0) {
        ...
        if (segmentLength == 0) {
            ...
            final double[] hiddenStateValues = hiddenStates.stream()
                    ...
                    .toArray();
            return IntStream.range(firstTargetIndex, firstTargetIndex + segmentLength)
                    .mapToDouble(targetIndex -> IntStream.range(0, hiddenStates.size())
     ...
     */
    public static <STATE extends ScalarProvider, TARGET extends Target> double calculateSegmentVariance(
            final int firstTargetIndex,
            final int segmentLength,
            final ForwardBackwardAlgorithm.Result<?, TARGET, STATE> fbResult) {
        if (segmentLength < 0) {
        ...
        if (segmentLength == 0) {
            ...
            final double[] hiddenStateValues = hiddenStates.stream()
                    .mapToDouble(STATE::getScalar).toArray();
    ...
    public static double logProbToPhredScore(final double rawLogProb, final boolean complement) {
        if (rawLogProb > MAX_LOG_PROB) {
            throw new GATKException(
                    String.format("numerical instability problem: the log-probability is too large: %g > 0.0 " +
                            "(with maximum tolerance %g)", rawLogProb, MAX_LOG_PROB));
        }
