 ...
 *
 * This class implements a number of automatic bookkeeping strategies for cacheing the results of computable nodes
 ...
 * Otherwise, only the cache status will go out of date. Unchanged nodes are passed as refernce to the new instance.
 * JVM's garbage collector will free up the memory for old cached nodes in sequantial computations.
 *
...
public class ImmutableComputableGraph implements Serializable {
    ...
    public ImmutableComputableGraph(@Nonnull final Collection<? extends CacheNode> nodesCollection,
        ...
        final Set<String> nodeKeys = nodesCollection.stream().map(CacheNode::getKey).collect(Collectors.toSet());
        if (nodeKeys.size() != nodesCollection.size()) {
            throw new IllegalArgumentException("The node list containes nodes with non-unique keys.");
        }
    ...
    private void assertNodeExists(final String nodeKey) {
        if (!cgs.getNodeKeysSet().contains(nodeKey)) {
            throw new IllegalArgumentException("The node \"" + nodeKey + "\" does not exist.");
        }
    }

    private void assertTagExists(final String tagKey) {
        if (!cgs.getNodeTagsSet().contains(tagKey)) {
            throw new IllegalArgumentException("The tag \"" + tagKey + "\" does not exist.");
        }
    }
