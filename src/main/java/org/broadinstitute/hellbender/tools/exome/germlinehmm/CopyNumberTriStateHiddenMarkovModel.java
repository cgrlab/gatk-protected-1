...
public class CopyNumberTriStateHiddenMarkovModel<D>
    ...
    public CopyNumberTriStateHiddenMarkovModel(@Nonnull final TargetLikelihoodCalculator<D> emissionProbabilityCalculator,
        ...
        this.targetLikelihoodCalculator = emissionProbabilityCalculator;
    }
    ...
    public double logTransitionProbability(final CopyNumberTriState currentState, final Target fromTarget,
        ...
        try {
            logProb = logTransitionProbabilityCache.logProbability((int) distance, nextState, currentState);
        } catch (NullPointerException ex) {
            System.out.println(distance + " " + currentState.name() + " " + fromTarget.getName() + " " + nextState.name() +
    ...
    public TargetLikelihoodCalculator<D> getTargetLikelihoodCalculator() {
        return targetLikelihoodCalculator;
    }

    public double getDeletionMean() {
        if (targetLikelihoodCalculator instanceof XHMMEmissionProbabilityCalculator) {
            return ((XHMMEmissionProbabilityCalculator) targetLikelihoodCalculator).deletionMean();
        } else {
            throw new UnsupportedOperationException("This method is only applicable to XHMM emission model. " +
                    "The current instance is initialized with a different emission model.");
        }
    }
