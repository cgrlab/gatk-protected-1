...
/**
 * This class computes a number of useful structural properties for an acyclric directed graph
 * specified by the immediate parents and children of the nodes.
 */
public final class ComputableGraphStructure {

    /**
     * This exception will be thrown if the graph is not acyclic
     */
    ...
    ComputableGraphStructure(@Nonnull final Set<String> nodeKeysSet,
        ...
        for (final String node : nodeKeysSet) {
            if (immediateDescendentsMap.get(node) == null ) {
                throw new IllegalArgumentException("The immediate descendents node map has null values.");
            }
            if (!nodeKeysSet.containsAll(immediateDescendentsMap.get(node))) {
                throw new IllegalArgumentException("The immediate descendents node map refers to unknown nodes.");
            }
            if (immediateParentsMap.get(node) == null ) {
                throw new IllegalArgumentException("The immediate parents node map has null values.");
            }
            if (!nodeKeysSet.containsAll(immediateParentsMap.get(node))) {
                throw new IllegalArgumentException("The immediate parents node map refers to unknown nodes.");
            }
        }
        ...
        for (final String node : nodeKeysSet) {
            ...
            /* sort by depth */
            allParentsIncludingTheNode.sort((L, R) -> depthsMap.get(L) - depthsMap.get(R));
        ...
        for (final String tag : nodeTagsSet) {
            ...
            /* sort by depth */
            allParentsIncludingTheNodesList.sort((L, R) -> depthsMap.get(L) - depthsMap.get(R));
        ...
        evalStrategyForAllNodes.sort((L, R) -> depthsMap.get(L) - depthsMap.get(R));
        ...
        for (final String mutNode : nodeKeysSet) {
            ...
            /* sort by depth */
            allInvolvedList.sort((L, R) -> depthsMap.get(L) - depthsMap.get(R));
