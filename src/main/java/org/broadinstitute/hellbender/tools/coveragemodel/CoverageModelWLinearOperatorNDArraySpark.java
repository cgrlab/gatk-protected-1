 ...
 */
public class CoverageModelWLinearOperatorNDArraySpark extends GeneralLinearOperator<INDArray> {

    private final Logger logger = LogManager.getLogger(CoverageModelWLinearOperatorNDArraySpark.class);
    ...
    private final JavaPairRDD<LinearSpaceBlock, CoverageModelEMComputeBlockNDArray> computeRDD;

    public CoverageModelWLinearOperatorNDArraySpark(@Nonnull final INDArray Z_ll,
                                                    @Nonnull final FourierLinearOperatorNDArray F_tt,
                                                    final int numTargets,
                                                    @Nonnull final JavaSparkContext ctx,
                                                    @Nonnull final JavaPairRDD<LinearSpaceBlock, CoverageModelEMComputeBlockNDArray> computeRDD,
    ...
    public INDArray operate(@Nonnull final INDArray W_tl) throws DimensionMismatchException {
        ...
        final Map<LinearSpaceBlock, INDArray> W_tl_map = CoverageModelSparkUtils.mapINDArrayToBlocks(targetSpaceBlocks, W_tl);
...
//                    final CoverageModelEMComputeBlockNDArray cb = p._1;
