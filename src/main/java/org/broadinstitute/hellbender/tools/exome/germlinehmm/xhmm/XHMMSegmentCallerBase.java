...
/**
 * Parent class for those tools that perform CNV segment calls based on a {@link XHMMModel}.
 *
...
public abstract class XHMMSegmentCallerBase extends CommandLineProgram {
    ...
    @ArgumentCollection
    protected XHMMArgumentCollection modelArguments =
            new XHMMArgumentCollection();
    ...
    private void runForwardBackwardAndViterbi(final XHMMModel model, final TargetCollection<Target> targets,
        ...
        for (int sampleIndex = 0; sampleIndex < inputCounts.columnNames().size(); sampleIndex++) {
            final List<XHMMEmissionData> emissionData = DoubleStream.of(inputCounts.counts()
                    .getColumn(sampleIndex))
