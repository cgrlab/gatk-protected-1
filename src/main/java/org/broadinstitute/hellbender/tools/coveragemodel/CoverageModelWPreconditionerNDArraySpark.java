 ...
 *
 */
public class CoverageModelWPreconditionerNDArraySpark extends GeneralLinearOperator<INDArray> {

    private final Logger logger = LogManager.getLogger(CoverageModelWPreconditionerNDArraySpark.class);
    ...
    public CoverageModelWPreconditionerNDArraySpark(@Nonnull final INDArray Q_ll,
...
//        linOpPairRDD = CoverageModelSparkUtils.rddFromINDArray(linOp, fourierSpaceBlocks, ctx, true);
        ...
        linOpPairRDD = ctx.parallelizePairs(
                CoverageModelSparkUtils.chopINDArrayToBlocks(fourierSpaceBlocks, linOp), fourierSpaceBlocks.size());
        linOpPairRDD.cache();
    }
    ...
    public INDArray operate(@Nonnull final INDArray W_tl)
        ...
        final Map<LinearSpaceBlock, INDArray> W_kl_map = CoverageModelSparkUtils.mapINDArrayToBlocks(fourierSpaceBlocks, W_kl);
        ...
        final JavaPairRDD<LinearSpaceBlock, INDArray> preconditionedWRDD = linOpPairRDD
                .mapToPair(p -> {
                    ...
                    final List<INDArray> linOpWList = IntStream.range(0, blockSize).parallel()
                            .mapToObj(k -> CoverageModelEMWorkspaceNDArrayUtils.linsolve(linOp_chunk.get(NDArrayIndex.point(k)),
...
//                            CoverageModelEMWorkspaceNDArrayUtils.linsolve(linOp.get(NDArrayIndex.point(k)),
