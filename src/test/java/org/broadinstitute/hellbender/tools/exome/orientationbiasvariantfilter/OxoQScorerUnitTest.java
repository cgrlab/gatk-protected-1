...
public class OxoQScorerUnitTest extends BaseTest {
    ...
    public void testBasicScoring() throws IOException {
        ...
        final Map<Pair<Character, Character>, Double> scoreMap = OxoQScorer.scoreOrientationBiasMetricsOverContext(mf.getMetrics());
        ...
        Assert.assertEquals(scoreMap.get(Pair.of('A', 'C')), 100.0, 1e-6);
        Assert.assertEquals(scoreMap.get(Pair.of('A', 'G')), 50.5788416297570, 1e-6);
        Assert.assertEquals(scoreMap.get(Pair.of('A', 'T')), 100.0, 1e-6);
        Assert.assertEquals(scoreMap.get(Pair.of('C', 'A')), 100.0, 1e-6);
        Assert.assertEquals(scoreMap.get(Pair.of('C', 'G')), 100.0, 1e-6);
        Assert.assertEquals(scoreMap.get(Pair.of('C', 'T')), 58.0641821538479, 1e-6);
    }
    ...
    public void testBasicCounting() throws IOException {
        ...
        final Map<Pair<Character, Character>, RealMatrix> countMap = OxoQScorer.countOrientationBiasMetricsOverContext(mf.getMetrics());
        ...
        Assert.assertEquals(countMap.get(Pair.of('A', 'C')).getRowVector(0).getL1Norm(), 2836660 + 2631203 + 240 + 246, 1e-1);
        Assert.assertEquals(countMap.get(Pair.of('A', 'C')).getRowVector(1).getL1Norm(), 2852491 + 333 + 2646654 + 297, 1e-1);
        Assert.assertEquals(countMap.get(Pair.of('A', 'G')).getRowVector(0).getL1Norm(), 2631203 + 416 + 2836660 + 481, 1e-1);
        Assert.assertEquals(countMap.get(Pair.of('A', 'G')).getRowVector(1).getL1Norm(), 2852491 + 404 + 2646654 + 397, 1e-1);
    }
}
